...
async function savePhoto(photoData) {
  try {
    ...
    request.input('imageBuffer', sql.VarBinary(sql.MAX), photoData.imageBuffer);
    const result = await request.query(`
      INSERT INTO Photos (title, description, location, date, category, isFavorite, imageBuffer, userId)
      VALUES (@title, @description, @location, @date, @category, @isFavorite, @imageBuffer, @userId);
...
async function updatePhoto(id, photoData, userId) {
  try {
    ...
    if (photoData.imageBuffer) {
      request.input('imageBuffer', sql.VarBinary(sql.MAX), photoData.imageBuffer);
      await request.query(`
        UPDATE Photos SET title=@title, description=@description, location=@location, date=@date, category=@category, isFavorite=@isFavorite, imageBuffer=@imageBuffer WHERE id=@id AND userId=@userId
      `);
    } else {
      await request.query(`
        UPDATE Photos SET title=@title, description=@description, location=@location, date=@date, category=@category, isFavorite=@isFavorite WHERE id=@id AND userId=@userId
      `);
    }
    return { success: true };
  } catch (error) {
